using System;
using TMPro;
using UnityEngine;

public class AutoSpeedScript : MonoBehaviour
{
    public GameObject statsObj;
    private Stats stats;
    public GameObject CashSound;
    protected int value;

    void Start()
    {
        stats = statsObj.GetComponent<Stats>();
    }

    public void Click()
    {
        if (stats.clicks >= value && stats.autospeed + 1 <= 40)
        {
            stats.autospeed += 1;
            stats.clicks -= value;
            setValue();
            CashSound.GetComponent<AudioSource>().Play();
        }
    }

    public int setValue()
    {
        value = (int)(200*(1-((float)stats.discounts/100))) * (int)Math.Round(Math.Pow(1.25, stats.autospeed));
        foreach (Transform component in GetComponentsInChildren<Transform>())
        {
            if (component.name == "Value")
            {
                component.GetComponent<TMP_Text>().text = value.ToString();
            }
            if (component.name == "Amount")
            {
                component.GetComponent<TMP_Text>().text = "x" + stats.autospeed.ToString();
            }
        }
        return value;
    }
}
