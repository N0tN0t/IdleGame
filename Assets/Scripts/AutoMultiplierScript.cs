...
public class AutoMultiplierScript : MonoBehaviour
{
    ...
    public int setValue()
    {
        value = (int)(2000*(1-((float)stats.discounts/100))) * (int)Math.Round(Math.Pow(1.35, stats.automultiplier));
        foreach (Transform component in GetComponentsInChildren<Transform>())
        {
            ...
            {
                component.GetComponent<TMP_Text>().text = "x" + stats.automultiplier.ToString();
            }
        }
        return value;
    }
